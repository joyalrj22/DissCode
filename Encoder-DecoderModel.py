...
from keras.layers import Input, Dense, concatenate, Concatenate, TimeDistributed, Bidirectional, LSTM, Activation, Flatten, Lambda, Reshape
from keras.optimizers import RMSprop
import h5py
import os
import sys

from CES_data__ import load_CES_data
...
def emotion_model(max_seq_len, num_features, learning_rate, num_units_1, num_units_2, dropout, num_targets):
    # Input layers
    encoderInput = Input(shape=(max_seq_len,num_features), name='in1')
    decoderInputArousal = Input(shape=(max_seq_len,1), name='in2')
    decoderInputValence = Input(shape=(max_seq_len,1), name='in3')
    decoderInputLiking = Input(shape=(max_seq_len,1), name='in4')
    decoderInput = Concatenate(axis=-1, name='kjj')([decoderInputArousal, decoderInputValence, decoderInputLiking])
    
    
    #only change we made was dropout
    encoder = LSTM(num_units_1, return_sequences=True, return_state=True, dropout=dropout, name='enc')
    #encoder_outs, fwd_h, fwd_c, bckwd_h, bckwd_c = encoder(encoderInput)
    encoder_outs, state_h, state_c = encoder(encoderInput)
    #state_h = Concatenate()([fwd_h, bckwd_h])
    #state_c = Concatenate()([fwd_c, bckwd_c])
    
    decoder = LSTM(num_units_2, return_sequences=True, return_state=True, dropout=dropout, name='dec')
    decoder_out = decoder(decoderInput, initial_state=[state_h, state_c])

    attention = ContextVector(name='cv')
    attention_out = attention([encoder_outs, decoder_out[0]])
    weighted_out = Concatenate(axis=-1, name='c1')([decoder_out[0], attention_out[0]])
    dense_softmax = Dense(3, activation='softmax', name='sm')
    softmax_out = dense_softmax(weighted_out)

    arousal_out = Dense(1, name='a')
    valence_out = Dense(1, name='v')
    liking_out = Dense(1, name='l')


    # Output layer (linear activation)
    outputs = []
    outputs.append(TimeDistributed(arousal_out, name='in1ff')(softmax_out))
    if num_targets>=2:
        outputs.append(TimeDistributed(valence_out, name='in1sdd')(softmax_out))
    if num_targets==3:
        outputs.append(TimeDistributed(liking_out, name='in1efrfv')(softmax_out))
    
    # Create and compile model
    rmsprop = RMSprop(lr=0.001)
    model   = Model(inputs=[encoderInput, decoderInputArousal, decoderInputValence, decoderInputLiking], outputs=outputs)
    model.compile(optimizer=rmsprop, loss=ccc_loss)  # CCC-based loss function


    encoder_t_inputs = Input(batch_shape=(1, max_seq_len, num_features), name='in11')
    #encoder_t_outs, fwd_t_h, fwd_t_c, bckwd_t_h, bckwd_t_c = encoder(encoder_t_inputs)
    encoder_t_outs, t_state_h, t_state_c = encoder(encoder_t_inputs)
    #t_state_h = Concatenate()([fwd_t_h, bckwd_t_h])
    #t_state_c = Concatenate()([fwd_t_c, bckwd_t_c])
    encoder_model = Model(inputs=[encoder_t_inputs], outputs=[encoder_t_outs,t_state_h, t_state_c])


    decoder_t_InputArousal = Input(batch_shape=(1,max_seq_len,1), name='in12')
    decoder_t_InputValence = Input(batch_shape=(1,max_seq_len,1), name='in13')
    decoder_t_InputLiking = Input(batch_shape=(1,max_seq_len,1), name='in4')

    encoder_t_states = Input(batch_shape=(1,max_seq_len, num_units_2), name='in15')
    decoder_t_init_state_h = Input(batch_shape=(1,num_units_2), name='in16')
    decoder_tt_init_state_h = Lambda(lambda x:x, name='state_layer1')(decoder_t_init_state_h)
    decoder_t_init_state_c = Input(batch_shape=(1,num_units_2), name='in17')
    decoder_tt_init_state_c = Lambda(lambda x:x, name='state_layer2')(decoder_t_init_state_c)
    decoderInputs = Concatenate(axis=-1, name='cc')([decoder_t_InputArousal, 
                                            decoder_t_InputValence,
                                            decoder_t_InputLiking])
    decoder_t_outs = decoder(decoderInputs, initial_state=[decoder_tt_init_state_h, decoder_tt_init_state_c])

    attention_t_out = attention([encoder_t_states, decoder_t_outs[0]])
    concat_softmax = Concatenate(axis=-1, name='c11')([decoder_t_outs[0], attention_t_out[0]])
    concat_softmax = dense_softmax(concat_softmax)
    d_arousal_out = TimeDistributed(arousal_out, name='td')(concat_softmax)
    d_valence_out = TimeDistributed(valence_out, name='td1')(concat_softmax)
    d_liking_out = TimeDistributed(liking_out, name='td2')(concat_softmax)
    decoder_model = Model(inputs=[decoder_t_InputArousal,
                                    decoder_t_InputValence,
                                    decoder_t_InputLiking,encoder_t_states, decoder_t_init_state_h, decoder_t_init_state_c],
                         outputs=[d_arousal_out, d_valence_out, d_liking_out , decoder_t_outs[1], decoder_t_outs[2]])

    return model, encoder_model, decoder_model

def main(features_folders=['Audio/audio_features_egemaps_xbow/', 'Visual/visual_features_xbow/'], path_output='predictions/', fileEnding=['.csv', '.csv']):
    ...
    print('Loading data ...')
    train_DE_x, train_HU_x, train_DE_y, train_HU_y, devel_DE_x, devel_HU_x, devel_DE_labels_original, devel_HU_labels_original, test_DE_x, test_HU_x, test_CN_x = load_CES_data(base_folder, features_folders, targets, test_available, fileEnding=fileEnding)
    ...
    #train_x = np.concatenate((train_DE_x, devel_DE_x), axis=0)
    #devel_x = np.concatenate((train_HU_x, devel_HU_x), axis=0)
    # Get some stats
    max_seq_len  = train_x.shape[1]  # same for all partitions
    num_features = train_x.shape[2]

    padded_devel_DE_labels_og = devel_DE_labels_original
    padded_devel_DE_labels_original = [[], [], []]
    for i in range(3):
        for j in range(len(padded_devel_DE_labels_og[i])):
            frames=[]
            for k in range(len(padded_devel_DE_labels_og[i][j])):
                frames.append([padded_devel_DE_labels_og[i][j][k][0]])
            for k in range(max_seq_len-len(padded_devel_DE_labels_og[i][j])):
                frames.append([0])
            padded_devel_DE_labels_original[i].append(frames)

    padded_devel_DE_labels_original = np.array(padded_devel_DE_labels_original)  


    
    padded_devel_HU_labels_og = devel_HU_labels_original
    padded_devel_HU_labels_original = [[], [], []]
    for i in range(3):
        for j in range(len(padded_devel_HU_labels_og[i])):
            padded_devel_HU_labels_og[i][j][0][0] = 0
            frames=[]
            for k in range(len(padded_devel_HU_labels_og[i][j])):
                frames.append([padded_devel_HU_labels_og[i][j][k][0]])
            for k in range(max_seq_len-len(padded_devel_HU_labels_og[i][j])):
                frames.append([0])
            padded_devel_HU_labels_original[i].append(frames)

    padded_devel_HU_labels_original = np.array(padded_devel_HU_labels_original) 
    ...
    for t in range(0,num_targets):
        ...
        #train_y.append( np.concatenate((train_DE_y[t], padded_devel_DE_labels_original[t]), axis=0) )
        devel_labels_original.append( devel_DE_labels_original[t] + devel_HU_labels_original[t] )
        #devel_labels_original.append( np.concatenate((train_HU_y[t], padded_devel_HU_labels_original[t]), axis=0) )
    ...
    for i in range(3):
        for j in range(len(decoder_devel_labels_og[i])):
            ...
            for k in range(max_seq_len-len(decoder_devel_labels_og[i][j])):
                frames.append([0])
    ...
    for i in range(3):
        for j in range(len(decoder_devel_DE_labels_og[i])):
            ...
            for k in range(max_seq_len-len(decoder_devel_DE_labels_og[i][j])):
                frames.append([0])
    ...
    for i in range(3):
        for j in range(len(decoder_devel_HU_labels_og[i])):
            ...
            for k in range(max_seq_len-len(decoder_devel_HU_labels_og[i][j])):
                frames.append([0])
    ...
    # Create model
    model, enc_model, dec_model = emotion_model(max_seq_len, num_features, learning_rate, num_units_1, num_units_2, dropout, num_targets)
    print(model.summary())
    ...
    while epoch <= num_epochs:
        model.fit([train_x, decoder_train_y[0], decoder_train_y[1], decoder_train_y[2]], train_y, batch_size=batch_size, initial_epoch=epoch-1, epochs=epoch)  # Evaluate after each epoch
        
        #print([type(devel_x), type(decoder_devel_labels_original[0]), type(decoder_devel_labels_original[1]), type(decoder_devel_labels_original[2])])
        # Evaluate on development partition
        ccc_iter = evaluate_devel(model, [devel_x, decoder_devel_labels_original[0], decoder_devel_labels_original[1], decoder_devel_labels_original[2]], devel_labels_original, shift, targets)
        ...
        for t in range(0, num_targets):
            if ccc_iter[t] > ccc_devel_best[t]:
                ...
                #dec_model.save_weights(targets[t]+'_decoder.hdf5', overwrite=True)
                save_model(model, targets[t]+'.hdf5')
                #file3 = h5py.File(targets[t]+'hdf5', 'w')
                #enc_model.save_weights(targets[t]+'_encoder.hdf5', overwrite=True)
            """
            if epoch==40:
                file1 = h5py.File(targets[t]+'_decoder.hdf5', 'w')
                weight = dec_model.get_weights()
                for i in range(len(weight)):
                    file1.create_dataset('weight'+str(i),data=weight[i])
                file1.close()

                file2 = h5py.File(targets[t]+'_encoder.hdf5', 'w')
                weight = enc_model.get_weights()
                for i in range(len(weight)):
                    file2.create_dataset('weight'+str(i),data=weight[i])
                file2.close()
            """
    ...
    if output_predictions_devel:
        ...
        for t in range(0, num_targets):
            if num_targets==1:
                pred_devel_DE.append( shift_labels_to_back( load_model(targets[t]+'.hdf5',compile=False, custom_objects={"ContextVector": ContextVector}).predict([devel_DE_x, decoder_devel_DE_labels_original[0], decoder_devel_DE_labels_original[1], decoder_devel_DE_labels_original[2]]), shift) )
                pred_devel_HU.append( shift_labels_to_back( load_model(targets[t]+'.hdf5',compile=False, custom_objects={"ContextVector": ContextVector}).predict([devel_HU_x, decoder_devel_HU_labels_original[0], decoder_devel_HU_labels_original[1], decoder_devel_HU_labels_original[2]]), shift) )
            else:
                pred_devel_DE.append( shift_labels_to_back( load_model(targets[t]+'.hdf5',compile=False, custom_objects={"ContextVector": ContextVector}).predict([devel_DE_x, decoder_devel_DE_labels_original[0], decoder_devel_DE_labels_original[1], decoder_devel_DE_labels_original[2]])[t], shift) )
                pred_devel_HU.append( shift_labels_to_back( load_model(targets[t]+'.hdf5',compile=False, custom_objects={"ContextVector": ContextVector}).predict([devel_HU_x, decoder_devel_HU_labels_original[0], decoder_devel_HU_labels_original[1], decoder_devel_HU_labels_original[2]])[t], shift) )
...
if __name__ == '__main__':
    main(features_folders=['Audio/audio_features_egemaps/', 'Audio/audio_features_mfcc/','Visual/visual_features/'], 
    fileEnding=['.csv', '.csv', '.csv'],
    path_output= 'AendecoderII/predictions/')

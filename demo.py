...
def extract_deep_visual():
    parser = argparse.ArgumentParser()
    parser.add_argument('--src_vid_path', default=os.getcwd()+'/video/', type=str) # define source video directory
    parser.add_argument('--openFace_bin_path', default='./OpenFace-master/build/bin/FeatureExtraction', type=str) # openFace implementation
    parser.add_argument('--orig_out_data_path', default='processed/', type=str)
    parser.add_argument('--dest_out_data_path', default='visual_features/', type=str) # define the output path
    parser.add_argument('--save_img', default=1) # save the aligned face images and .csv files or not
    parser.add_argument('--save_feat', default=1) # save landmarks and feature files or not
    parser.add_argument('--Res_feat', default=1) # Extracting Aff-wild ResNet feature
    parser.add_argument('--VGG_feat', default=0) # Extracting Aff-wild ResNet feature
    parser.add_argument('--Res_reg_feat', default=0) # Extracting Aff-wild ResNet feature
    ...
    #./home/joyal/Documents/Baseline_features_extraction/visual_features/aligned_face/temp_video_aligned/

    for i in range(0, len(vid_list)):
        print(vid_list[i])

        # implementing OpenFace for face alignment
        #subprocess.check_output([args.openFace_bin_path,' -f ',args.src_vid_path+vid_list[i], ' -oc', 'MJPG'])
        subprocess.call([args.openFace_bin_path+' -f '+args.src_vid_path+vid_list[i]+ ' -oc MJPG'], shell=True)
        
        #os.system(args.openFace_bin_path+' -f '+args.src_vid_path+vid_list[i]+ ' -oc MJPG')
        #os.system( ) #+ ' -oc MJPG'
        ...
        #if not os.path.exists(face_img_path[1:]):
        #    print(face_img_path)
        ...
        if args.Res_feat == 1:
            os.system('LD_LIBRARY_PATH=/usr2/local/cuda-9.0/lib64:/usr2/local/cuda-9.0/lib64/stubs/:$LD_LIBRARY_PATH python3.6 OpenFace-master/build/bin/feature_extraction/feature_extraction_RESNET.py --input_file '+ out_csv_path + ' --save_file '+feat_out_Name_res)
        if args.VGG_feat == 1:
            os.system('LD_LIBRARY_PATH=/usr2/local/cuda-9.0/lib64:/usr2/local/cuda-9.0/lib64/stubs/:$LD_LIBRARY_PATH python3.6 OpenFace-master/build/bin/feature_extraction/feature_extraction_VGG.py --input_file '+ out_csv_path + ' --save_file '+ feat_out_Name_vgg) 
        
        if args.Res_reg_feat == 1:
            os.system('LD_LIBRARY_PATH=/usr2/local/cuda-9.0/lib64:/usr2/local/cuda-9.0/lib64/stubs/:$LD_LIBRARY_PATH python3.6 OpenFace-master/build/bin/feature_extraction/feature_extraction_RESNET_reg.py --input_file '+ out_csv_path + ' --save_file '+ feat_out_Name_res_reg)

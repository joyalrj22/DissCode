...
from keras.layers import Input, Dense, Masking, SimpleRNN, concatenate,TimeDistributed, Bidirectional, LSTM, Activation, RepeatVector, Permute, Flatten
...
def emotion_model(max_seq_len, num_features, learning_rate, num_units_1, num_units_2, dropout, num_targets):
    # Input layers
    audioInput = Input(shape=(max_seq_len[0],num_features[0]))
    visualInput = Input(shape=(max_seq_len[1],num_features[1]))
    
    #LSTM encoder for input
    audioEncoded = Bidirectional(LSTM(num_units_1, return_sequences=True, dropout=dropout, recurrent_dropout=dropout))(audioInput)

    audioAttention = Dense(1, activation='tanh')(audioEncoded)
    audioAttention = Flatten()(audioAttention)
    audioAttention = Activation('softmax')(audioAttention)
    audioAttention = RepeatVector(audioInput.shape[2])(audioAttention)
    audioAttention = Permute([2,1])(audioAttention)
    
    w_audio = layers.multiply([audioInput, audioAttention])



    visualEncoded = Bidirectional(LSTM(num_units_1, return_sequences=True, dropout=dropout, recurrent_dropout=dropout))(visualInput)

    visualAttention = Dense(1, activation='tanh')(visualEncoded)
    visualAttention = Flatten()(visualAttention)
    visualAttention = Activation('softmax')(visualAttention)
    visualAttention = RepeatVector(visualInput.shape[2])(visualAttention)
    visualAttention = Permute([2,1])(visualAttention)
    
    w_visual = layers.multiply([visualInput, visualAttention])
    #do two attention mechanisms!
    #attentiveInput = TimeDistributed(Dense(1, activation='sigmoid'))()

    # Masking zero input - shorter sequences
    
    # 1st layer
    
    net = LSTM(num_units_1, return_sequences=True, dropout=dropout, recurrent_dropout=dropout)(concatenate([w_audio, w_visual]))
    ...
    model   = Model(inputs=[audioInput, visualInput], outputs=outputs)
...
def main(audio_features_folders=['Audio/audio_features_egemaps_xbow/'], visual_features_folders=['Visual/visual_features_xbow/'], path_output='predictions/', audioFileEndings=['.csv'], visualFileEndings=['.csv']):
    ...
    batch_size    = 17       # Full-batch: 68 sequences
    ...
    while epoch <= num_epochs:
        model.fit([audio_train_x, visual_train_x], train_y, batch_size=batch_size, initial_epoch=epoch-1, epochs=epoch)  # Evaluate after each epoch
        
        # Evaluate on development partition
        ccc_iter = evaluate_devel(model, [audio_devel_x, visual_devel_x], devel_labels_original, shift, targets)
    ...
    if output_predictions_devel:
        ...
        for t in range(0, num_targets):
            if num_targets==1:
                pred_devel_DE.append( shift_labels_to_back( load_model(targets[t]+'.hdf5',compile=False).predict([audio_devel_DE_x, visual_devel_DE_x]), shift) )
                pred_devel_HU.append( shift_labels_to_back( load_model(targets[t]+'.hdf5',compile=False).predict([audio_devel_HU_x, visual_devel_HU_x]), shift) )
            else:
                pred_devel_DE.append( shift_labels_to_back( load_model(targets[t]+'.hdf5',compile=False).predict([audio_devel_DE_x, visual_devel_DE_x])[t], shift) )
                pred_devel_HU.append( shift_labels_to_back( load_model(targets[t]+'.hdf5',compile=False).predict([audio_devel_HU_x, visual_devel_HU_x])[t], shift) )
...
if __name__ == '__main__':
    main(audio_features_folders=['Audio/audio_features_egemaps_functionals/', 'Audio/audio_features_egemaps_xbow/', 'Audio/audio_features_mfcc_functionals/', 'Audio/audio_features_mfcc_xbow/', 'Audio/audio_features_deepspectrum/', 'Audio/audio_features_deepspectrum_II/'], 
            visual_features_folders=['Visual/visual_features_functionals/', 'Visual/visual_features_xbow/', 'Visual_Resnet/visual_features_ResNet/', 'VGG/'], 
            path_output= 'AEarly_Mixed/predictions/', audioFileEndings=['.csv', '.csv', '.csv', '.csv', '.csv','.csv'], visualFileEndings=['.csv', '.csv', '_res.txt','_vgg.txt'])
